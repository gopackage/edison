...
type mraa_pincapabilities_t struct {
	valid     bool // Is the pin valid at all
	gpio      bool // Is the pin gpio capable
	pwm       bool // Is the pin pwm capable
	fast_gpio bool // Is the pin fast gpio capable
	spi       bool // Is the pin spi capable
	i2c       bool // Is the pin i2c capable
	aio       bool // Is the pin analog input capable
	uart      bool // Is the pin uart capable
}

func mraa_pincapabilities(valid, gpio, pwm, fast_gpio, spi, i2c, aio, uart int) mraa_pincapabilities_t {
	return mraa_pincapabilities_t{
		valid != 0,
		gpio != 0,
		pwm != 0,
		fast_gpio != 0,
...
type pininfo_t struct {
	...
	capabilities mraa_pincapabilities_t
	gpio         pin_t
	pwm          pin_t
	aio          pin_t
	// TODO(stephen) mmap
...
func PwmInit(pin int) (*pwm_context, error) {
	...
	if plat.pins[pin].capabilities.gpio {
