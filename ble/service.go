...
// NewService creates a new bluetooth GATT service.
func NewService(name string, uuid *UUID, primary bool, c *Characteristic) *Service {
	s := &Service{}

	return s
}

// Characteristic represents a bluetooth GATT characteristic.
type Characteristic struct {
}

// NewCharacteristic creates a bluetooth GATT characteristic.
func NewCharacteristic(name string, uuid *UUID, flags []string, descriptor *Descriptor) *Characteristic {
	c := &Characteristic{}
	return c
}

// Descriptor represents a bluetooth GATT descriptor.
type Descriptor struct {
}

// NewDescriptor creates a new descriptor and initializes it.
func NewDescriptor(name, description string, flags []string, handler *Handler) *Descriptor {
	d := &Descriptor{}
	return d
}
